function createModel(nGPU)
  ...
  local function conv_pw(nInput, nOutput)
    mobilenet:add(nn.SpatialConvolution(nInput,nOutput,1,1,1,1,0,0))
    mobilenet:add(nn.SpatialBatchNormalization(nOutput,1e-3))
    mobilenet:add(nn.ReLU(true))
  end

  -- depth-wise convolution
  local function conv_dw(nInput, sW, sH)
    mobilenet:add(nn.SpatialDepthWiseConvolution(nInput,1,3,3,sW,sH,1,1))
  ...
  mobilenet:add(nn.SpatialAveragePooling(4, 4, 1, 1))
